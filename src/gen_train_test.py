...
def localize_dataset(data, num_rows=NUM_ROWS, num_cols=NUM_COLS):
    ...
    for fname, metadata in data.items():
        ...
        with open('localize.txt', 'w') as fout:
           json.dump(object['locations'],fout)
...
def main():
    scene_files = glob.glob(osp.join('/localscratch/cater_dataset/max2action', 'scenes/*.json'))    
    ...
    for dset_name, dset_fn in dataset_gen_fns.items():
        ...
        if not (osp.exists(train_shuf_txt_outfpath) and
            ...
            with open(osp.join(this_output_dir, 'metadata.pkl'), 'wb') as fout:
